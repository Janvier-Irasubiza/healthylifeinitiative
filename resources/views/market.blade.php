...
<x-ht-layout>
    <div class="body">
        ...
        <div class="ctgr-hold mt-4 mb-3 text-center py-5">
            <h1 class="c-name mb-3">Seems like there's nothing in Market for now!</h1>

            <div class="mt-4 px-2">
                <a href="{{ route('index') }}" class="buttons-cstm fw-500">GO HOME</a>
            </div>

            @foreach($ctgrs as $ctgr)
            <div class="mt-4 px-2">
                <a href="{{ route('products', ['category' => $ctgr -> slag]) }}" class="buttons-cstm fw-500">GO TO {{ $ctgr -> category }}</a>
            </div>
            @endforeach
        </div>
        @endif
