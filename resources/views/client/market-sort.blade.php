        ...
        <div class="d-flex justify-content-between gap-3">
             ...
             <div class="mt-2 mb-4 mrkt-ctgr">
                ...
                <a href="{{ route('client.sort-market', ['ctgr' => $ctgr -> id]) }}" class="add-btn inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md bg-white dark:bg-gray-800 focus:outline-none transition ease-in-out duration-150 mb-1" style="color: black; font-size: 13px">
                    <div><strong>{{ $ctgr -> category }}</strong></div>
                </a>
                @else
                <a href="{{ route('client.sort-market', ['ctgr' => $ctgr -> id]) }}" class="add-btn inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md bg-white dark:bg-gray-800 focus:outline-none transition ease-in-out duration-150 mb-1" style="color: black; font-size: 13px">
            ...
            @else
                    ...
                    @php
                        $ctgrs = DB::table('categories') -> where('id', '!=', $categorie -> id) -> get();
                    @endphp

                <div class="ctgr-hold mt-4 mb-3 text-center py-5">
                    <h1 class="c-name mb-3">Seems like there's nothing in {{ $categorie -> category }} for now!</h1>

                    <div class="mt-4 px-2">
                        <a href="{{ route('client.dashboard') }}" class="buttons-cstm fw-500">GO TO DASHBOARD</a>
                    </div>

                    @foreach($ctgrs as $ctgr)
                    <div class="mt-4 px-2">
                        <a href="{{ route('client.sort-market', ['ctgr' => $ctgr -> id]) }}" class="buttons-cstm fw-500">GO TO {{ $ctgr -> category }}</a>
                    </div>
                    @endforeach
                </div>
            @endif
