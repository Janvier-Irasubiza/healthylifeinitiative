    ...
    <div class="body">
        ...
        <div class="product-info">
            <div class="img-cont">
                ...
                <div class="item-img">
                    <img src="{{ asset('images/products') }}/{{ $product -> pic6 }}" alt="">
                </div>
            </div>
            ...
            <div class="item-content w-full">
                ...
                <div class="w-full desc">
                    ...
                    <form action="{{ route('client.confirm-order') }}"> @csrf 
        ...
        <div class="ctgr-hold mt-5 mb-5">
             <h1 class="c-name mb-3">Related products</h1>
            <div class="products justify-content-start gap-3">
            @foreach($related as $rel)
            <a href="{{ route('client.product', ['product' => $rel -> uuid]) }}" class="card item-card">
                    <div class="item-card-img d-flex justify-content-center">
                        <img class="w-full" src="{{ asset('images/products') }}/{{ $rel -> poster }}" alt="poster">
                    </div>
                    <div class="item-card-desc gap-2">
                        <div><h4 class="c-head">{{ $rel -> name }}</h4></div>
                        <div class="d-flex gap-3">
                            @php
                                $likedProducts = DB::table('liked_products')->where('liked_by', Auth::user() -> id)->where('product', $rel->id)->get();
                                $cartProducts = DB::table('cart')->where('client', Auth::user() -> id)->where('product', $rel->id)->get();
                            @endphp
                            ...
                            <span class="cart-icon" data-product-id="{{ $rel->id }}"><i class="fa-solid fa-cart-shopping take-action" style="color: #f0ad4e"></i></span>
                            @else
