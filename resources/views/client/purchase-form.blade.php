...
<x-ht-clt-layout>
    <div class="body">
        ...
        <div class="path mt-1 mb-2">
            Category / {{ $productInfo['name'] }} / Order
        </div>

        <form action="{{ route('client.make-order') }}" method="POST"> @csrf

        <input class="w-full" type="hidden" name="product_id" value="{{ $productInfo['id'] }}">
        <input class="w-full" type="hidden" name="client" value="{{ Auth::user() -> id }}">
        ...
        <div class="product-info mb-5">
            ...
            <div class="desc mt-2">
                    ...
                    <p class="m-0 off-price"><span>{{ number_format($productInfo['unit_price']) }} </span> &nbsp; Save &nbsp; {{ number_format($productInfo['promo_price'] * $productInfo['quantity']) }} RWF </p>
                    @else
                    <p class="price mt-3">
                        {{ number_format($productInfo['price']) }} RWF
                    </p>
